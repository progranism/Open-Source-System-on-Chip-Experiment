...
// Changed from bin2hex to bin2mif - William Heatley 2012
...
int main(int argc, char *argv[])
{
	...
	if((argc != 4) && (argc != 5)) {
		fprintf(stderr, "Usage: bin2mif <infile> <outfile> <size> [16]\n");
		return 1;
	}
	pad = atoi(argv[3]);
	if(pad <= 0) {
	...
	fprintf (fdo, "WIDTH=%d;\nDEPTH=%d;\n\n", mode16 ? 16 : 32, pad);
	fprintf (fdo, "ADDRESS_RADIX=UNS;\nDATA_RADIX=HEX;\n\nCONTENT BEGIN\n");

	if(mode16) {
		while(1) {
			if(fread(w, 2, 1, fdi) <= 0) break;
			fprintf(fdo, "\t%d\t:\t%02hhx%02hhx;\n", addr++, w[1], w[0]);
			pad--;
		}
	} else {
		while(1) {
			if(fread(w, 4, 1, fdi) <= 0) break;
			fprintf(fdo, "\t%d\t:\t%02hhx%02hhx%02hhx%02hhx;\n", addr++, w[0], w[1], w[2], w[3]);
			pad--;
		}
	}
	fclose(fdi);
